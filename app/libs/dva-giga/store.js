...
export const addAsyncModel = function addAsyncModel(m) {
...
const enhancers = [
  ...
  devtools({
    actionSanitizer: (action) => {
      // console.log(action);
      return action.type === 'FILE_DOWNLOAD_SUCCESS' && action.data ? { ...action, data: '<<LONG_BLOB>>' } : action;
    },
...
// 创建 store
export const store = createStore(combineReducers({ ...reducers, ...extraReducers }), compose(...enhancers));
